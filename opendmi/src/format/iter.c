        ...
        const void            *value)
{
    ...
    iter->attr = attr;
    iter->mask = dmi_attribute_get_uint(attr, value);
    iter->next = 0;
...
const dmi_format_flag_t *dmi_format_set_iter_next(dmi_format_set_iter_t *iter)
{
    ...
    const dmi_attribute_t *attr = iter->attr;
    size_t width = attr->value.size * CHAR_BIT;

    while (iter->next < width) {
        size_t id = iter->next++;

        const char *code = dmi_code_lookup(attr->params.values, (int)id);
        if (code == nullptr)
            continue;
        ...
        iter->flag = (dmi_format_flag_t){
            .id    = id,
            .code  = code,
            .name  = dmi_name_lookup(attr->params.values, (int)id),
